...
export const pollReducer = createReducer({
    ...
    'Poll.Admin.advanceQuestion': (poll) => {
        // let existingResponses = (poll.active.responses || [])
        // let nextId = (poll.active.id || -1) + 1
        // let nextQuestion = poll.questions.find(q => q.id >= nextId)

        // // Love the syntax for updating objects using 'immutable' !
        // return fromJS(poll)
        //     .update('responses', r => r.push(...existingResponses))
        //     .update('active', () => nextQuestion )
        //     .toJS()
    }
}, newPoll())
...
export const getStore = (docId) => {
    ...
    let store = makeStoreFromReducer(pollReducer)
